    ...
    <div className="flex flex-col bg-card gap-3 sm:gap-5 p-2 sm:p-2.5 border border-[var(--glass-border)] rounded-[16px] sm:rounded-[20px]">
      ...
      {showBalance && (
        <div className="bg-[rgba(37,37,40,255)] backdrop-blur-md bg-opacity-20 p-4 sm:p-6 rounded-lg">
          <h3 className="text-gray-400 text-xs sm:text-sm mb-1 sm:mb-2">Доступно для вывода</h3>
          ...
          >
            {type === 'deposit' ? (
              <>
                ...
                <h4 className="font-semibold text-sm sm:text-base mb-1">{method.name}</h4>
              ...
              <div className="flex flex-col items-center">
                ...
                <p className="text-xs sm:text-sm opacity-80">{method.limits}</p>
                {method.commission && (
                  <p className="text-xs opacity-60 mt-0.5 sm:mt-1">Комиссия: {method.commission}</p>
                )}
              </div>
            )}
          </div>
        ...
        <div className="bg-[rgba(37,37,40,255)] backdrop-blur-md bg-opacity-20 p-4 sm:p-6 rounded-lg">
          <div className="flex items-center space-x-2 sm:space-x-3 mb-4 sm:mb-6">
            ...
            <div>
              <h3 className="text-yellow-400 text-base sm:text-lg font-semibold">
                {type === 'deposit' 
                  ? `${selectedMethod.name}${selectedMethod.popular ? ' (Crypto Bot)' : ''}` 
                  : `Вывод на ${selectedMethod.name}`}
              </h3>
              <p className="text-yellow-300 text-xs sm:text-sm">
                {type === 'deposit' ? (
                  <>
                    <span className='text-yellow-200'>Минимум:</span> {selectedMethod.limits.split('-')[0]} | 
                    <span className='text-yellow-200'> Максимум:</span> {selectedMethod.limits.split('-')[1]}
                  </>
                ) : (
                  `Лимиты: ${selectedMethod.limits}`
                )}
              </p>
            </div>
          ...
          <div className="space-y-3 sm:space-y-4">
            ...
            <button className="w-full px-3 sm:px-4 py-2 sm:py-3 bg-[rgba(29,29,34,255)] border border-[rgba(58,56,53,255)] rounded-[14px] sm:rounded-[18px] button-yellow text-sm sm:text-base">
              {type === 'deposit' ? 'Пополнить' : 'Вывести средства'}
            </button>
